...
function setupScene() {
  renderer = new THREE.WebGLRenderer({antialising: true, alpha: true, preserveDrawingBuffer: true, alpha: true});
  
  renderer.setSize(window.innerWidth, window.innerHeight);
  renderer.setPixelRatio(window.devicePixelRatio);
  
  // Enable alpha blending for trail effect
   renderer.autoClearColor = false; // Don't auto-clear the canvas
  ...
  let rect = new THREE.Mesh(new THREE.PlaneGeometry(100, 100), new THREE.MeshBasicMaterial({color: 0x000000, transparent: true, opacity: 0.15}));
  ...
  scene.background = null; // Remove solid background for trail effect
...
function animate(now) {
  ...
  // Create trail effect using WebGL
